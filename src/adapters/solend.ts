...
export async function fetch(connection: Connection): Promise<ProtocolRates> {
  ...
  const rates: AssetRate[] = market.reserves.map((reserve) => {
    return {
      ...
      borrowAmount: reserve
        .stats!.totalBorrowsWads.div(new BN(WAD))
        .div(new BN(10 ** reserve.config.decimals))
        .toNumber(),
      borrowRate: reserve.stats!.borrowInterestAPY,
      depositAmount: reserve
        .stats!.totalDepositsWads.div(new BN(WAD))
        .div(new BN(10 ** reserve.config.decimals))
        .toNumber(),
