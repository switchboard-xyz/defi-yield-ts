...
export const FarmPoolParser = (pubkey: PublicKey, info: AccountInfo<Buffer>|null):Detail<any> => {
    if (info==null){
    ...
    if (buffer.length===STAKE_INFO_LAYOUT.span){
        farmPoolInfo = STAKE_INFO_LAYOUT.decode(buffer)
    } else if (buffer.length === STAKE_INFO_LAYOUT_V4.span){
        farmPoolInfo = STAKE_INFO_LAYOUT_V4.decode(buffer)
    }
    const details = {
        pubkey,
        account: {
