...
export const AmmOpenOrdersLayoutParser = (pubkey: PublicKey, info: AccountInfo<Buffer>|null):Detail<AmmOpenOrders> => {
    if (info==null){
        throw new Error("Info can not be null")
    }
    const buffer = Buffer.from(info.data);
    const reserve = AMM_OPEN_ORDERS_LAYOUT.decode(buffer);
    const details = {
        pubkey,
        account: {
            ...info,
